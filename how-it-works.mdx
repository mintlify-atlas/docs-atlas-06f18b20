...
The **Implementer** sub-agent supports an optional Test-Driven Development workflow. When enabled, it follows the RED-GREEN-REFACTOR cycle:

1. **RED** - Write a failing test first that describes expected behavior from spec scenarios
2. **GREEN** - Write minimum code needed to make the test pass
3. **REFACTOR** - Clean up structure, naming, and remove duplication while keeping tests passing

Enable TDD mode in `openspec/config.yaml`:
...
Or let the implementer detect it from your codebase (test files alongside source). This cycle repeats for every task, ensuring test coverage grows with implementation.

<Info>
See the [TDD Workflow Guide](/guides/tdd-workflow) for detailed examples and best practices.
</Info>
